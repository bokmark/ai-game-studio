---
name: "setup-engine"
description: "Configure the project's game engine and version. Pins the engine in AGENTS.md / CLAUDE.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data."
---
...
**Question 1 — Prior experience** (ask this first, always, via the question tool):
...
**Question 2 — Target platform** (ask this second, always, via the question tool — platform eliminates or heavily weights engines before any other factor):
...
5. Use the question tool to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use the question tool with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
## 4. Update Technology Stack (AGENTS.md + CLAUDE.md)

> **Dual-platform**: the Technology Stack section is identical in `AGENTS.md`
> (Codex) and `CLAUDE.md` (Claude Code). Always update both files with the
> same content.
...
Record the choice. It determines the AGENTS.md / CLAUDE.md template, naming conventions, specialist routing, and which agent is spawned for code files throughout the project.

---

Read `AGENTS.md` and show the user the proposed Technology Stack changes.
Ask: "May I write these engine settings to both `AGENTS.md` and `CLAUDE.md` (identical sections)?"
...
After updating both AGENTS.md and CLAUDE.md, create or update `.claude/docs/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use the question tool:
...
## 8. Update Engine Reference Pointers

**Codex side (`AGENTS.md`)**: no update needed. Its Document Map references
`docs/engine-reference/` generically and instructs agents to check the pinned
version before using engine APIs.

**Claude side (`CLAUDE.md`)**: the `@` import is Claude Code syntax. Ask:
"May I update the `@` import in `CLAUDE.md` to point to the new engine reference?"
...
> **Dual-platform**: agent definitions exist twice —
> `.claude/agents/<name>.md` (Claude) and `.codex/agents/<name>.toml`
> (Codex, inside `developer_instructions`). Apply the same addition to both.
...
- Always show the user what you're about to change before making AGENTS.md / CLAUDE.md edits
- If WebSearch returns ambiguous results, show the user and let them decide
- When the user chose **GDScript**: copy the GDScript template from Appendix A1 exactly (into both AGENTS.md and CLAUDE.md). NEVER add "C++ via GDExtension" to the Language field. GDScript projects may use GDExtension, but it is not a primary project language. The `godot-gdextension-specialist` in the routing table is available for when native extensions are needed — it does not make C++ a project language.
...
### A1. Technology Stack Templates (written to both AGENTS.md and CLAUDE.md)
