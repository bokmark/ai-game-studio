...
If they differ, use the question tool before proceeding:
...
specialist to spawn via subagent.
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via subagent. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
