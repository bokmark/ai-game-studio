...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use the question tool:
...
Present the full proposed inventory to the user in conversation. Then use the question tool:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use the question tool: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use the question tool:
...
Present the full identified list to the user. Use the question tool:
...
After presenting all specs, use the question tool:
