...
a **confirm/adjust** prompt using the question tool — not open-ended questions.
...
decisions remain, present each one as a separate the question tool with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via Task to validate the drafted ADR:
   ...
   - Spawn `[primary specialist]` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
5. **Write approval** — Use the question tool:
...
After the ADR is written (and registry optionally updated), close with the question tool.
