...
- `qa-lead` — Strategy, planning, classification, sign-off
- `qa-tester` — Test case writing and bug report writing
...
Present the qa-lead's full strategy to the user, then use the question tool:
...
Use the question tool per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use the question tool for each:
...
After each FAIL result: use the question tool to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
