...
This is the first thing the user sees. Use the question tool with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not the question tool (it's an open response).
2. Acknowledge the concept, then use the question tool to offer two paths:
...
**If it does not exist**: Use the question tool:
...
After presenting the recommended path, use the question tool to ask the user which step they'd like to take first. Never auto-run the next skill.
