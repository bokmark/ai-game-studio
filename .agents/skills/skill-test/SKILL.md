...
existing skill/hook/template architecture. (Dual-platform note: the Claude
mirror at `.claude/skills/` is out of scope for the Codex-side checks.)
...
- `static all` → run 7 structural checks on all skills (list `.agents/skills/*/SKILL.md`)
...
### Check 1 — Frontmatter Fields (Codex format)
The YAML frontmatter block must contain exactly:
- `name:` — matches the skill's directory name
- `description:` — non-empty, third-person, includes when-to-use triggers

**FAIL** if either is absent or mismatched.
**FAIL** if Claude-only keys are present (`allowed-tools`, `model`,
`argument-hint`, `user-invocable`, `context`, `agent`, `isolation`,
`disable-model-invocation`) — these indicate an unmigrated or regressed skill.
Optional companion metadata lives in `agents/openai.yaml`, never in frontmatter.
...
**FAIL** if the skill instructs file writes (Write/Edit operations, "write to
`path`", "create the file") but no ask-before-write language is found.
...
Every `$skill-name` mentioned in the body must resolve to a directory under
`.agents/skills/`. Every agent name mentioned in a spawn/delegate instruction
must resolve to a `.codex/agents/<name>.toml` file.

**WARN** for each dangling `$skill` reference (may be a planned skill).
**FAIL** for each dangling agent reference in a delegation instruction.

### Check 7 — No Claude-Only Syntax
The body must not contain Claude Code runtime syntax:
- `/command` skill references (must be `$skill-name`)
- `Task tool`, `subagent_type`, `AskUserQuestion` (delegation must use
  neutral subagent language)
- Instructions to write to `CLAUDE.md` (Codex-side skills write to
  `AGENTS.md`; dual-file updates say "AGENTS.md / CLAUDE.md")

**FAIL** for any occurrence — these break invocation or mislead the agent.
...
Check 6 — Reference Integrity:   PASS (12 $skill refs, 3 agent refs resolve)
Check 7 — No Claude-Only Syntax: PASS
...
List `.agents/skills/*/SKILL.md` to get the complete list of skills.
