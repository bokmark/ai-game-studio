...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use the question tool:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   the question tool for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use the question tool to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   the question tool. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use the question tool with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use the question tool:
...
- Surface any disagreements between agents to the user via the question tool
...
- Present the specialists' proposals to the user for review via the question tool
...
- The main session presents the agent's output to the user via the question tool
- The user decides; the main session writes to file
...
2. **the question tool** at every decision point (Explain -> Capture pattern):
...
> open a fresh session and run `$design-system [system-name]` — it will
