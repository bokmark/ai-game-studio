...
- **Check 1 fail** → name/description missing or mismatched, or Claude-only
  frontmatter keys present (`allowed-tools`, `model`, `argument-hint`, etc.)
- **Check 2 fail** → how many phases found vs. minimum required
- **Check 3 fail** → no verdict keywords anywhere in the skill body
- **Check 4 fail** → skill instructs file writes but has no ask-before-write language
- **Check 5 warn** → no follow-up or next-step section at the end
- **Check 6 warn/fail** → dangling `$skill` or agent reference
- **Check 7 fail** → Claude-only syntax present (`/cmd` refs, `Task tool`,
  `subagent_type`, `AskUserQuestion`, `CLAUDE.md` write instructions)
