...
Use the question tool for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second the question tool:
...
After all file writes are complete, use the question tool for a closing widget.
