...
**Argument:** `$ARGUMENTS` (blank = ask user via the question tool)
...
- **No argument** — use the question tool:
...
then ask two questions together using the question tool:
...
- Use the question tool for scope selection when no argument is provided.
