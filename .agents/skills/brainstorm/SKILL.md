...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use the question tool** at key decision points throughout brainstorming:
   ...
   the question tool to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an the question tool with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use the question tool for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab the question tool with these exact tab labels:
...
Present all three. Then use the question tool to capture the selection.
...
Ask these as the question tool calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use the question tool:
...
If the user selects B, C, or D, make the revision, then use the question tool again:
...
Collect both verdicts, then present them together using a two-tab the question tool:
...
- **Target platform**: Use the question tool — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use the question tool — "Do you already have an engine you work in?"
...
5. Use the question tool for write approval:
...
If [B]: ask which section to revise using the question tool with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use the question tool — "Ready to write the updated concept document?"
