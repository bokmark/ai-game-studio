...
3. If multiple in-progress stories are found, use the question tool:
...
### Manual verification with confirmation (use the question tool)
...
Batch up to 4 manual verification questions into a single the question tool call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via the question tool
...
- `lean` → use the question tool before proceeding:
...
Present the verdict to the user. If CONCERNS, surface them via the question tool:
...
Use the question tool before writing anything:
...
- Use the question tool for the code review prompt and for batching manual
